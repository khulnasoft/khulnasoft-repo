...
*/
typedef NSUInteger MTSyslogMessageFacility;
NS_ENUM(MTSyslogMessageFacility) {
...
*/
typedef NSUInteger MTSyslogMessageSeverity;
NS_ENUM(MTSyslogMessageSeverity) {
...
*/
typedef NSUInteger MTSyslogMessageMaxSize;
NS_ENUM(MTSyslogMessageMaxSize) {
...
*/
- (void)setHostname:(NSString* _Nullable)hostName;
...
@discussion    The process id is a value that is included in the message, having no interoperable meaning,
 except that a change in the value indicates there has been a discontinuity in syslog reporting.  This is an
 optional value.
*/
- (void)setProcessId:(NSString* _Nullable)procId;
...
@discussion    The message id should identify the type of message.  For example, a firewall might use the
 message id "TCPIN" for incoming TCP traffic and the message id "TCPOUT" for outgoing TCP traffic. Messages
 with the same message id should reflect events of the same semantics.This is an optional value.
*/
- (void)setMessageId:(NSString* _Nullable)msgId;
...
@discussion    If not specified, the maximum size of a syslog message (header + event message) is set to
 MTSyslogMessageMaxSize480 because this is the minimum maximum message size, a syslog server must
 support.
*/
...
*/
- (NSString* _Nullable)messageString;
