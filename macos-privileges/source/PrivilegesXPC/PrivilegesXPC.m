    ...
    // Part of XPCServiceProtocol.  Called by the app to install the helper tool.
{
    ...
    success = SMJobBless(
        ...
        CFSTR("corp.khulnasoft.privileges.helper"),
        self->_authRef,
        &error
    );
    ...
    // Without this authorization will fail because the app is sandboxed.
{
    ...
    [self.queue addOperationWithBlock:^{
        ...
        [[self.helperToolConnection remoteObjectProxyWithErrorHandler:^(NSError *proxyError) {
            os_log_with_type(OS_LOG_DEFAULT, OS_LOG_TYPE_ERROR, "KHULNASOFTCorp: Failed to connect to helper tool: %{public}@ / %{public}d", [proxyError domain], (int)[proxyError code]);
            reply(nil, nil);
