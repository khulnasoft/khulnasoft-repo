...
- (void)changeAdminGroup:(NSString*)userName remove:(BOOL)remove
{
    [self connectAndExecuteCommandBlock:^(NSError *connectError) {
          ...
          if (connectError) {
              os_log_with_type(OS_LOG_DEFAULT, OS_LOG_TYPE_ERROR, "KHULNASOFTCorp: %{public}@", connectError);
              [self displayErrorNotificationAndExit];
              
          } else {
              
              [[self.helperToolConnection remoteObjectProxyWithErrorHandler:^(NSError *proxyError) {
                  os_log_with_type(OS_LOG_DEFAULT, OS_LOG_TYPE_ERROR, "KHULNASOFTCorp: %{public}@", proxyError);
                  [self displayErrorNotificationAndExit];
                  ...
                  if (error) {
                      os_log_with_type(OS_LOG_DEFAULT, OS_LOG_TYPE_ERROR, "KHULNASOFTCorp: Unable to change privileges: %{public}@", error);
                      [self displayErrorNotificationAndExit];
                
                  } else {
                  
                      // send a notification to update the Dock tile
                      [[NSDistributedNotificationCenter defaultCenter] postNotificationName:@"corp.khulnasoft.PrivilegesChanged"
                                                                                     object:userName
                                                                                   userInfo:nil
...
{
    os_log_with_type(OS_LOG_DEFAULT, OS_LOG_TYPE_ERROR, "KHULNASOFTCorp: %{public}@", errorMessage);
    ...
    [[self.xpcServiceConnection remoteObjectProxyWithErrorHandler:^(NSError *proxyError) {
        ...
        if (!installError) {
            ...
            os_log(OS_LOG_DEFAULT, "KHULNASOFTCorp: The helper tool has been successfully installed");
            ...
            os_log_with_type(OS_LOG_DEFAULT, OS_LOG_TYPE_ERROR, "KHULNASOFTCorp: Installation of the helper tool failed: %{public}@", installError);
...
{
    if ([[sender identifier] isEqualToString:@"corp.khulnasoft.button.continue"]) {
...
- (IBAction)actionButtonPressed:(id)sender
{
    if (_autoApplyPrivileges) {
        ...
        if ([[sender identifier] isEqualToString:@"corp.khulnasoft.button.default"] && !_autoApplyPrivileges) {
    ...
    if (self.xpcServiceConnection == nil) {
        ...
        self.xpcServiceConnection.invalidationHandler = ^{
            ...
            [[NSOperationQueue mainQueue] addOperationWithBlock:^{
                ...
                os_log(OS_LOG_DEFAULT, "KHULNASOFTCorp: XPC connection invalidated");
            }];
        };
    ...
    if (self.helperToolConnection == nil) {
        ...
        self.helperToolConnection.invalidationHandler = ^{
            ...
            [[NSOperationQueue mainQueue] addOperationWithBlock:^{
                ...
                os_log(OS_LOG_DEFAULT, "KHULNASOFTCorp: Helper tool connection invalidated");
            }];
        };
        #pragma clang diagnostic pop
